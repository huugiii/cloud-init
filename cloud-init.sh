...
sed -i "s/^#PasswordAuthentication .*/PasswordAuthentication yes/" "$SSH_CONFIG" || echo "PasswordAuthentication yes" >> "$SSH_CONFIG"
...
log "Connect using: ssh -p ${SSH_PORT} ${ADMIN_USER}@<server_ip>"
